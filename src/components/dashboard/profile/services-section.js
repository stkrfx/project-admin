...
import { Sparkles, Plus, Trash2, Video, Phone, MessageSquare, MapPin, Clock, DollarSign } from "lucide-react";
...
export function ServicesSection({ services, setServices }) {
  ...
  const addService = () => {
    setServices([
      ...services, 
      { name: "", type: "video", duration: 60, price: 0, currency: "AUD", description: "" }
    ]);
  };
  ...
  const updateService = (index, field, val) => {
    ...
    // Ensure numeric values for price/duration
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pr-8">
                                    <div className="space-y-1.5">
                                        <Label className="text-xs font-bold text-zinc-500 uppercase tracking-wide">Service Name</Label>
                                        <Input 
                                            value={service.name} 
                                            onChange={(e) => updateService(i, 'name', e.target.value)} 
                                            placeholder="e.g. Initial Consultation" 
                                            className="h-10 font-semibold border-zinc-200 focus:ring-amber-500"
                                        />
                                    </div>
                                    <div className="space-y-1.5">
                                        <Label className="text-xs font-bold text-zinc-500 uppercase tracking-wide">Type</Label>
                                        <Select value={service.type} onValueChange={(val) => updateService(i, 'type', val)}>
                                            <SelectTrigger className="h-10 border-zinc-200 focus:ring-amber-500">
                                                <SelectValue />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {SERVICE_TYPES.map(type => (
                                                    <SelectItem key={type.value} value={type.value}>
                                                        <div className="flex items-center gap-2">
                                                            <type.icon className="h-3.5 w-3.5 text-zinc-500" />
                                                            {type.label}
                                                        </div>
                                                    </SelectItem>
                                                ))}
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="relative">
                                            ...
                                            <Input 
                                                ...
                                                className="h-9 pl-9 pr-8 bg-white border-zinc-200 text-sm"
                                            />
                                            <span className="absolute right-3 top-2.5 text-xs text-zinc-400 font-medium">min</span>
                                        </div>
                                    </div>
                                    <div className="space-y-1">
                                        <Label className="text-[10px] font-bold text-zinc-400 uppercase">Price (AUD)</Label>
                                        <div className="relative">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <Textarea 
                                        ...
                                        placeholder="What can clients expect from this session?" 
                                        className="h-20 text-sm resize-none border-zinc-200 focus:ring-amber-500"
                                    />
                                </div>
                            </div>
                        </div>
